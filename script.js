...
function renderFavs(){
  favListEl.innerHTML = ""; favCountEl.textContent = String(state.favs.length);
  for(const f of state.favs){
    ...
    const title = Object.assign(document.createElement("div"), { className:"grow", textContent:f.strDrink });
    const openBtn = Object.assign(document.createElement("button"), { className:"btn", textContent:"Open" });
    const rmBtn = Object.assign(document.createElement("button"), { className:"btn", textContent:"Remove" });
    openBtn.onclick = () => openDrink(f);
    rmBtn.onclick = () => { state.favs = state.favs.filter(x=>x.idDrink!==f.idDrink); persistFavs(); renderFavs(); };
    li.append(img,title,openBtn,rmBtn); favListEl.appendChild(li);
  }
}
function toggleSave(drink){
  const idx = state.favs.findIndex(x=>x.idDrink===drink.idDrink);
  if(idx>=0) state.favs.splice(idx,1); else state.favs.unshift(minify(drink));
  const seen = new Set();
...
async function loadRandom(){
  ...
  try{
    ...
    state.current = d; renderRandomCard(d);
    statusEl.textContent = "Press “New Drink” to shuffle again.";
  }catch(err){ console.error(err); statusEl.textContent="Couldn’t load a drink. Please try again."; }
  finally{ setBusy(false); updateSaveButtons(); }
}
...
function renderRandomCard(d){
  ...
  ingList.innerHTML = ""; for(const item of ingredientsList(d)){ const li=document.createElement("li"); li.textContent=item; ingList.appendChild(li); }
...
searchForm.addEventListener("submit", async (e) => {
  e.preventDefault(); const term = searchInput.value.trim(); if(!term) return;
  await doSearch(term);
});
randomBtn.addEventListener("click", async () => {
  const res = await fetch("https://www.thecocktaildb.com/api/json/v1/1/random.php");
  const data = await res.json(); renderSearchCards(data.drinks ? [data.drinks[0]] : []);
});
...
async function doSearch(term){
  ...
  }catch(err){ console.error(err); statusEl.textContent="Something went wrong. Please try again."; }
